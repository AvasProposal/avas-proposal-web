...
export default function LoanCalculator() {
  ...
  const tenureYearsNumber = Number(tenureYears) || 0;
  const monthlyRevenueNumber = Number(monthlyRevenue) || 0;
  const monthlyExpensesNumber = Number(monthlyExpenses) || 0;

  const monthlyRate = interestRateNumber / 100 / 12;
  const numberOfPayments = tenureYearsNumber * 12;

  const emi =
    numberOfPayments === 0
      ? 0
      : monthlyRate === 0
      ? loanAmountNumber / numberOfPayments
      : (loanAmountNumber *
          monthlyRate *
          Math.pow(1 + monthlyRate, numberOfPayments)) /
        (Math.pow(1 + monthlyRate, numberOfPayments) - 1);

  const netOperatingIncome = monthlyRevenueNumber - monthlyExpensesNumber;
  const dscr = emi > 0 ? netOperatingIncome / emi : 0;
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-10 shadow-sm">
      ...
      <p className="mt-3 text-sm text-slate-600">
        Estimate EMI, net cashflow, and approximate DSCR to assess approval
        readiness.
      </p>
      ...
      <div className="mt-12 space-y-4 border-t border-slate-200 pt-8 text-sm">
      ...
      <p className="mt-6 text-xs leading-relaxed text-slate-500">
        ...
        quality, collateral structure, and lender-specific risk assessment
        criteria.
      </p>
